...
{% highlight php linenos %}
<?php
    $entity = App\Model\Item::getEntity();
...
{% highlight php linenos %}
<?php
    $entity = App\Model\Item::getEntityFromPost(get_post());
...
For additional information on each of these methods, have a look at the  page.
...
Returns the current, wrapped, `wp_query` object. Chainable.


### $this->date(array $dateQuery)

Applies a date query to the querier. Follow `WP_Query`'s documentation to learn how to format the parameters. Chainable.

### $this->orderby(string $orderBy)

Defines fields on which to order the current query. Chainable.

### $this->direction(string $dir)

Defines the direction on which the `orderby()` field will be sorted. Chainable.

### $this->status(string $status)

Shorthand for setting conditions on the `post_status` column. Chainable.

### $this->where(string $field, mixed $value)

The default catch all wrapper for `WP_Query` parameters. Chainable.

### $this->orWhere(string $field, mixed $value)

Allows the branching of different, still relatively simple, conditions in the same Wordpress query. Chainable.

### $this->limit(integer $qty)

Sets the maximum number of results the current query may return. Chainable.

### $this->offset(integer $offset)

Sets the starting offset of the current queries result set. Chainable.

### $this->byAuthorID(integer $offset)

Adds a 'author' condition to the current query. Chainable.
...
This is useful for returning a list of `[ID => post_title]` elements.
